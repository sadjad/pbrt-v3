    ...
    for (uint64_t root_index = nodeCount; root_index-- > 0;) {
        ...
        while (true) {
            ...
            if (best_node.nPrimitives == 0) {
                ...
                uint64_t leftNodeAdditionalSize = GetProxyBytes(nodeProxies[best_node_index + 1]);
                unordered_set<const ProxyBVH *> leftSubtreeProxies = *(subtreeProxies[best_node_index + 1]);
                uint64_t leftSubtreeAdditionalSize = GetProxyBytes(subtreeProxies[best_node_index + 1]);

                unordered_set<const ProxyBVH *> rightNodeProxies = *(nodeProxies[best_node.secondChildOffset]);
                uint64_t rightNodeAdditionalSize = GetProxyBytes(nodeProxies[best_node.secondChildOffset]);
                unordered_set<const ProxyBVH *> rightSubtreeProxies = *(subtreeProxies[best_node.secondChildOffset]);
                uint64_t rightSubtreeAdditionalSize = GetProxyBytes(subtreeProxies[best_node.secondChildOffset]);
                ...
                cut.emplace_back(best_node_index + 1, leftNodeProxies, leftSubtreeProxies,
                                 leftNodeAdditionalSize, leftSubtreeAdditionalSize);
                cut.emplace_back(best_node.secondChildOffset, rightNodeProxies, rightSubtreeProxies,
...
vector<uint32_t> ProxyDumpBVH::DumpTreelets(bool root, bool inlineProxies) const {
    ...
    for (uint32_t treeletID = 0; treeletID < allTreelets.size(); treeletID++) {
        ...
        // Get meshes for instances. The assumption here is that every proxy contains
        // unique meshes, which seems reasonable
        vector<protobuf::TriangleMesh> proxyMeshes;
        unordered_map<const ProxyBVH *, vector<uint32_t>> proxyMeshIndices;
        ...
        if (inlineProxies) {
            for (const ProxyBVH *proxy : *(treelet.proxies)) {
                ...
                for (int i = 0; i < numMeshes; i++) {
                    protobuf::TriangleMesh tm;
                    readers[0]->read(&tm);
                    proxyMeshes.emplace_back(move(tm));
                    proxyMeshIndices[proxy].push_back(i);
                }
        ...
        uint32_t numTriMeshes = trianglesInTreelet.size() + proxyMeshes.size();
        ...
        if (inlineProxies) {
            for (const ProxyBVH *proxy : *(treelet.proxies)) {
                auto &indices = proxyMeshIndices[proxy];
                for (auto &idx : indices) {
                    uint32_t sMeshId = global::manager.getNextId(ObjectType::TriangleMesh);
                    proxyMeshes[idx].set_id(sMeshId);
                    proxyMeshes[idx].set_material_id(0);
                    writer->write(proxyMeshes[idx]);
                    idx = sMeshId;
                }
            }
        }
