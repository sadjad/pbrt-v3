...
namespace pbrt {
    ...
    Optional<Mapping> DemandScheduler::distributeTreelets(const Mapping currentMapping, WorkerStats& stats, uint32_t numWorkers) {
        ...
        std::map<TreeletId, uint64_t> demandMap;
        ...
        std::map<TreeletId, float> proportionalDemand;
        for (auto& kv: demandMap) {
            proportionalDemand[kv.first] = (float)(kv.second) / (float)(totalDemand);
        }
        ...
        for (auto& kv: demandMap) {
            ...
            allocation[kv.first] += (uint64_t)(proportionalDemand[kv.first] * float(availableWorkers));
        }
        Mapping newMapping = newAssignments(currentMapping, allocation);

        return Optional<Mapping>(move(newMapping));
    }

    /* Minimizes worker movement */
    Mapping DemandScheduler::newAssignments(const Mapping currentMapping, const Allocation newAlloc) {
        Mapping newMapping;
        Allocation currentAlloc = calculateAllocation(currentMapping);
        std::map<TreeletId, int32_t> allocationDif;
        ...
        // iterate through the mapping (treelet -> set of workers) and take away workers as needed


        return newMapping;
    }

    Allocation DemandScheduler::calculateAllocation(const Mapping mapping) {
        Allocation alloc;
        for (auto &kv: mapping) {
